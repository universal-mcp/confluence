...
from universal_mcp_confluence.app import Confluence

@pytest.fixture
def app_instance():
    """Provides a Confluence instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return Confluence(integration=mock_integration)

def test_universal_mcp_confluence_app_initialization(app_instance):
    """
    Test that the Confluence instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "confluence", "Confluence instance has unexpected name."


def test_universal_mcp_confluence_tool_docstrings_format(app_instance):
    """
    Test that each tool method in Confluence has a well-formatted docstring,
...
def test_universal_mcp_confluence_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in Confluence is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
